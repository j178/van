import base64
import io
import os
import random
import string

import arrow
from nose.tools import *

try:
    from fanfou_sdk import van
    from fanfou_sdk.van import Fan, Status, User, Stream, Event
except ImportError:
    import van
    from van import Fan, Config, Status, User, Photo, Stream, Event

CONSUMER_KEY = 'b55d535f350dcc59c3f10e9cf43c1749'
CONSUMER_SECRET = 'e9d72893b188b6340ad35f15b6aa7837'

RAW_PHOTO = b'\njGatJXOqKcXpuf/Z\n'


def random_str(length=10):
    return ''.join(random.choice(string.ascii_letters) for _ in range(length))


def auth():
    username = os.environ['XAUTH_USERNAME']
    password = os.environ['XAUTH_PASSWORD']
    fan = Fan(CONSUMER_KEY, CONSUMER_SECRET)
    fan.xauth(username, password)
    return fan


fan = auth()


class TestAuth:
    def test_xauth(self):
        assert isinstance(fan._oauth_token, dict)


class TestAPI:
    def setup(self):
        self.fan = fan  # type:Fan
        self.me = self.fan.me  # type:User

    def test_user_api(self):
        assert_is_not_none(self.me.id)
        assert_is_not_none(self.me.name)
        assert_is_not_none(self.me.location)
        assert_is_not_none(self.me.screen_name)
        assert_is_not_none(self.me.unique_id)
        assert_is_not_none(self.me.gender)
        assert_is_instance(self.me.created_at, arrow.Arrow)

        assert_is_not_none(self.me.followers)
        assert_is_not_none(self.me.followers_id)
        assert_is_not_none(self.me.friends)
        assert_is_not_none(self.me.friends_id)
        assert_is_not_none(self.me.photos)
        assert_is_not_none(self.me.favorites)
        assert_is_not_none(self.fan.follow_requests)
        assert_is_not_none(self.fan.draft_box)
        assert_is_not_none(self.fan.blocked_users)
        assert_is_not_none(self.fan.blocked_users_id)

    def test_follow(self):
        other = User.from_id(self.fan, 'chengyujun')  # type:User

        self.fan.follow(other)
        assert_true(self.me.relationship(other)[1])
        self.fan.unfollow(other)
        assert_false(self.me.relationship(other)[1])

    def test_block(self):
        other = User.from_id(self.fan, 'chengyujun')  # type:User

        self.fan.block(other)
        assert_true(self.me.relationship(other)[0])
        assert_true(self.fan.is_blocked(other))
        self.fan.unblock(other)
        assert_false(self.me.relationship(other)[0])

    def test_status(self):
        status = self.fan.update_status(status=random_str())
        photo = io.BytesIO(base64.decodebytes(RAW_PHOTO))
        with photo:
            photo_status = self.fan.update_status(status=random_str(), photo=photo)
            photo.seek(0)

            assert_is_not_none(status.text)
            assert_is_not_none(status.context)
            assert_is_instance(status.created_at, arrow.Arrow)

            status.reply(random_str())
            status.reply(random_str(), photo=photo)
            photo.seek(0)

            status.repost(random_str())
            status.repost(random_str(), photo=photo)

        assert_true(status.favorite().favorited)
        assert_false(status.unfavorite().favorited)


class TestTimeline:
    def setup(self):
        self.fan = fan
        self.me = self.fan.me  # type:User
        self.tl = self.me.timeline

    def test_read(self):
        r = self.tl.read(10)
        assert_equal(len(r), 10)
        assert_equal(self.tl.tell(), 10)

        self.tl.read(10)
        assert_equal(self.tl.tell(), 20)

        ori_ptr = self.tl.tell()
        r = self.tl.read(1 << 32)
        assert_equal(self.tl.tell(), ori_ptr + len(r))

        r = self.tl.read()
        assert_equal(len(r), 0)

        self.me.statues.read()
        self.fan.replies.read()
        self.fan.mentions.read()
        self.fan.public_timeline.read()

    def test_call(self):
        tl = self.tl
        assert_is_not_none(tl.fetch(count=10))

    def test_seek(self):
        tl = self.tl
        tl.rewind()
        tl.read(count=60)
        assert_equal(tl.tell(), 60)

        tl.rewind()
        assert_equal(tl.tell(), 0)
        assert_equal(tl.seek(10), 10)
        assert_raises(ValueError, tl.seek, -1)
        assert_equal(tl.seek(1 << 32), len(tl) - 1)

        tl.rewind()
        tl.seek(10)
        assert_equal(tl.seek(10, 1), 20)
        assert_equal(tl.seek(-10, 2), len(tl) - 10)

        tl.rewind()
        tl.seek(10)
        assert_equal(tl.seek(-5, 1), 5)
        assert_equal(tl.seek(1 << 32, 1), len(tl) - 1)

        tl.rewind()
        assert_equal(tl.seek(-(1 << 32), 2), 0)
        assert_raises(ValueError, tl.seek, 1, 2)


class TestStream:
    def setup(self):
        self.fan = fan
        self.s = Stream(self.fan)
        self.s.start()
        self.i = 0

    def teardown(self):
        self.s.join(10)
        self.s.stop()

    def test_api(self):
        @self.s.on(Event.ALL, ttl=3)
        def test_ttl(evt):
            self.i += 1
            assert_less_equal(self.i, 3)

        assert_equal(len(self.s._listeners), 1)

        @self.s.on(Event.MESSAGE | Event.FAV)
        def test_combined_events(evt):
            assert_in(evt.type, [Event.MESSAGE, Event.FAV])

        assert_equal(len(self.s._listeners), 2)

        @self.s.on(Event.HEART_BEAT)
        def _(evt):
            assert_is_instance(evt, Event)
            assert_equal(evt.object, r'\r\n')
